...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.order.update', ['order' => $order]) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <select id="status" class="form-control @error('status') is-invalid @enderror"
                ...
                @foreach ($statuses as $status)
                    <option value="{{ $status->id }}" @if ($order->status->id == $status->id)
                        selected
                            ...
                            <td class="talbe__data talbe__data--action">
                                <div class="form-group">
                                    ...
                                    <input id="amount" type="number"
                                           ...
                                           value="{{ old('amount') ?? ($order->books->where('id', $book->id)->first()->pivot->amount ?? 0) }}"
                                           required
                                           autocomplete="amount"
                                           placeholder="0"
                                           min="0">
